...
import {AuthResponse, PaginatedResponse, PaginationParams, Response} from './type'

export async function signup(data: Partial<SignupForm>) {
  return await apiClient.post<Response<string>, Partial<SignupForm>>(`/auth/signup`, {
    body: data,
    withAuth: true
  })
}
...
export async function login(data: Partial<LoginForm>) {
  return await apiClient.post<Response<string>, Partial<LoginForm>>(`/auth/login`, {
    body: data,
    withAuth: true
  })
}
...
export async function logout() {
  return await apiClient.post<Response<string>, unknown>(`/auth/logout`, {
    withAuth: true
  })
}
