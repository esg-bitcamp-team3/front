    ...
    <>
      ...
      <Flex direction="row" justify="space-between" gap={6} mb={8}>
        ...
        <Card.Root
          ...
          <Card.Body>
            온실가스 배출량(tCO2eq) = 사용량 x 순발열량 x 온실가스별 배출계수 x 산화계수 x
            온실가스 등가계수 x 10^-6
            <br /> <br />
            <text font-Size="1px">
            ...
            <br />
            <text font-Size="1px">*산화계수 = 1 </text>
          </Card.Body>
        </Card.Root>
          ...
          <Card.Body>
            <Box overflowY="auto" maxHeight="500px" pr={2}>
              ...
              <Flex fontWeight="bold" bg="gray.100" p={2} borderTopRadius="md">
                ...
                <Box flex="1">CH4 배출계수 (에너지)</Box>
                <Box flex="1">CH4 배출계수 (제조업,건설업업)</Box>
                <Box flex="1">CH4 배출계수 (상업공공,기타)</Box>
              </Flex>
              ...
              {fuels?.map(fuel => (
                <Flex
                  ...
                  <Box flex="1">{fuel.calorificValue?.ncv ?? '-'}</Box>
                  <Box flex="1">
                    {fuel.emissionFactor?.efCO2_commercial_public ?? '-'}
                  </Box>

                  <Box flex="1">{fuel.emissionFactor?.efCH4_energy ?? '-'}</Box>
                  <Box flex="1">
                    {fuel.emissionFactor?.efCH4_manufacturing_construction ?? '-'}
                  </Box>
                  <Box flex="1">{fuel.emissionFactor?.efCH4_domestic_etc ?? '-'}</Box>
                </Flex>
              ))}
            </Box>
          </Card.Body>
