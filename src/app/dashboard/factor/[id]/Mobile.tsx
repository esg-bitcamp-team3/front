    ...
    <>
      ...
      <Flex direction="row" justify="space-between" gap={6} mb={8}>
        ...
        <Card.Root
          ...
          <Card.Body>
            온실가스 배출량(tCO2eq) = ∑(사용량 x 순발열량 x 온실가스별 배출계수 x 산화계수
            x 온실가스 등가계수 x 10^-9)
            <br /> <br />
            <text font-Size="1px">
            ...
            <Box overflowY="auto" maxHeight="500px" pr={2}>
              ...
              {fuels?.map(fuel => (
                <Flex
                  ...
                  <Box flex="1">{fuel.emissionFactor?.efCO2_mobile ?? '-'}</Box>

                  <Box flex="1">{fuel.emissionFactor?.efCH4_mobile ?? '-'}</Box>
                  <Box flex="1">{fuel.emissionFactor?.efN2O_mobile ?? '-'}</Box>
                </Flex>
              ))}
            </Box>
          </Card.Body>
