...
const HandsontableComponent: React.FC<HandsontableProps> = ({
  data = [[]],
  colHeaders = [
    '선택',
    '연도',
    ...
    try {
      ...
      tableData.forEach((row, rowIndex) => {
        ...
        formattedData.push({
          ...
          serialNumber: row[1] || '',
          facilityName: row[2] || '',
          emissionActivity: row[3] || '',
          activityData: activityDataDropdownMap.get(row[4]) || row[4] || '',
          total: parseFloat(row[6]) || 0,
          uncertainty: parseFloat(row[7]) || 0,
          data1: parseFloat(row[8]) || 0,
          data2: parseFloat(row[9]) || 0,
          data3: parseFloat(row[10]) || 0,
          data4: parseFloat(row[11]) || 0,
          data5: parseFloat(row[12]) || 0,
          data6: parseFloat(row[13]) || 0,
          data7: parseFloat(row[14]) || 0,
          data8: parseFloat(row[15]) || 0,
          data9: parseFloat(row[16]) || 0,
          data10: parseFloat(row[17]) || 0,
          data11: parseFloat(row[18]) || 0,
          data12: parseFloat(row[19]) || 0
        })
      })
